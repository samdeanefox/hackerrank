...
def makeAnagram(a, b):
    ...
    for letter in a:
        loc = b.find(letter)
        # If not in b, increment removals
        # No need to actually remove from a :)
        if loc == -1:
            removals += 1
        # If it is there, remove from b and proceed
        else:
            b = b[:loc] + b[loc+1:]
    # Add the length of whatever is left in b to removals
    return removals + len(b)
